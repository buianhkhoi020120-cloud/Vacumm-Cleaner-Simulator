...
class HillClimbing_Node:
    def __init__(self, state, parent=None, action=None):
        self.state = state  # Định dạng: ((rx, ry), tuple_các_vị_trí_rác)
        self.parent = parent
        self.action = action
        self.value = 0      # Giá trị đánh giá: Value = -h(n)


class HillClimbing_Vaccum:
    ...
    def _calculate_value(self, pos, dirt_tuple):
        ...
        # Dùng min() để tìm khoảng cách tới hạt rác gần nhất thay vì sum()
    ...
    def solve(self):
        ...
        current_node = HillClimbing_Node(initial_state, parent=None, action="START")
        current_node.value = self._calculate_value(current_node.state[0], current_node.state[1])
        # Khởi tạo mảng lưu vết hành trình cho UI
        nodes_journey = [current_node]
        while True:
            # Nếu đạt đích (hết rác), TRẢ VỀ Current_State thành công
            ...
            for action in actions:
                ...
                child_node = HillClimbing_Node(child_state, parent=current_node, action=action)
                child_node.value = self._calculate_value(child_node.state[0], child_node.state[1])
                # SO SÁNH VALUE: Nếu tốt hơn trạng thái hiện tại
                if child_node.value > current_node.value:
                    # Current_State = Next_State
                    current_node = child_node
                    nodes_journey.append(current_node)
                    found_better = True
                    break # LẬP TỨC THOÁT VÒNG LẶP (Tiếp tục vòng lặp lớn - Quay lại bước 2)
            if not found_better:
